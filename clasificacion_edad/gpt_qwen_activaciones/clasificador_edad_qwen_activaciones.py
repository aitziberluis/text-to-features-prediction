...
ACTIVATIONS_DIR = "/hdd/aitziber.l/activaciones_qwen_edad"
...
BALANCE_CONFIGS = [{"name": "sin_balanceo", "use_class_weights": False},{"name": "balanceo_manual", "use_class_weights": True},{"name": "undersampling", "use_class_weights": False},
]
# output
OUTPUT_DIR = "modelos/edad_qwen_activaciones"
...
class _Collate:
    ...
    def __call__(self, batch):
        ...
        tokens = self.tokenizer(textos,max_length=self.context_len,truncation=True,padding=True,  # padding dinamico al maximo del batchreturn_attention_mask=True,return_tensors="pt",
        )
...
def cargar_datos_edad() -> pd.DataFrame:
    ...
    df, _ = preparar_dataset_para_edad(path_comentarios=PATH_COMENTARIOS,path_autores=PATH_AUTORES,max_comments=MAX_COMMENTS,solo_edad_conocida=True,
    )
...
def _extraer_y_guardar_activaciones(df: pd.DataFrame) -> None:
    ...
    _trunk, _, _keep = truncar_modelo_para_hookpoint(model, HOOKPOINT)
    model._trunk = _trunk
    print(f"Modelo truncado a las primeras {_keep} capas para hook '{HOOKPOINT}'")
    ...
    loader = DataLoader(dataset,batch_size=EXTRACT_BATCH_SIZE,num_workers=EXTRACT_NUM_WORKERS,collate_fn=collate,pin_memory=True,shuffle=False,persistent_workers=False,)
    ...
    try:
        with torch.no_grad():
            for step, (input_ids, attention_mask, original_idx) in enumerate(loader):
                ...
                acts = captured["act"].float()

                # promovido a fp32 para evitar overflow al sumar a lo largo de seq
                mask = attention_mask.to(torch.float32)            # (bs, seq)
                ...
                last_np = np.nan_to_num(last_tok.float().cpu().numpy(), nan=0.0, posinf=0.0, neginf=0.0)
...
def entrenar_comentario(
    ...
    n = len(y_train)
    batch_size = 16384  # subido de 4096 -> 16384 (RAM trivial, 4x menos overhead)
...
def main():
    ...
    if has_author and authors is not None:
        ...
        for pooling_name in USER_POOLINGS:
            ...
            for bal_cfg in BALANCE_CONFIGS:
                if bal_cfg["name"] == "undersampling":
                    # submuestrear usuarios al tamaño de la clase minoritaria
    ...
    print("Resumen de resultados con eval")
    ...
    from _clasificador_utils import (
        select_best_per_level,print_best_per_level_eval,print_best_per_level_test,)
