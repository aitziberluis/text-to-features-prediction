"""
Clasificador de genero usando representaciones de SAE (version balanceada).
...
def cargar_dataset_genero() -> Dataset:
    """Carga comentarios + autores con genero usando funcion centralizada."""
    ...
    # Garantia explicita: solo comentarios con genero conocido (m/f), sin NaN en gender_clean.
    ...
    print(f"\n[VERIFICACION] Generos presentes (solo m/f): {dist}")
    print(f"[VERIFICACION] Nulos en gender_clean: {df_comentarios_con_genero['gender_clean'].isna().sum()}")
...
def _dividir_indices_estratificados(
    labels: np.ndarray,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Devuelve indices train/eval/test manteniendo proporcion de clase."""
...
def _pool_features(
    ...
    """Agrega activaciones sparse por token en una representacion por texto."""
...
def iterar_features_sae(
    ...
    """Genera batches (features, labels) sin acumular toda la matriz en RAM."""
...
def _evaluar_split(nombre_split: str, y_true: np.ndarray, y_pred: np.ndarray) -> None:
    """Imprime metricas completas para un split."""
...
def _buscar_umbral_optimo(
    ...
    """Busca umbral para clase male que mejore equilibrio en eval."""
...
def _aplicar_umbral_al_modelo(clf: SGDClassifier, threshold_male: float) -> None:
    """Ajusta el intercepto para que predict use el umbral elegido en vez de 0.5."""
    ...
    # Metadatos para inspeccion posterior
...
def entrenar_clasificador_streaming_balanceado(
    dataset: Dataset,
    sae: Sae,
    model: AutoModelForCausalLM,
    tokenizer,
    ...
    """Entrena en streaming y ajusta umbral para mejorar clase minoritaria."""
    ...
    print("\nEntrenando clasificador incremental balanceado (SGD + log_loss)...")
    print(f"Train batches por epoca: {math.ceil(len(train_dataset) / FEATURE_BATCH_SIZE):,}")

    first_batch = True
    classes = np.array([0, 1], dtype=np.int64)

    for epoch in range(TRAIN_EPOCHS):
        print(f"\nEpoca {epoch + 1}/{TRAIN_EPOCHS}")
        for x_batch, y_batch in iterar_features_sae(
            ...
            tokenizer=tokenizer,
            pooling_mode=pooling_mode,
            desc=f"Train balanceado ({pooling_mode})",
        ):
    ...
    for x_batch, y_batch in iterar_features_sae(
        ...
        prob_male_eval.append(proba[:, 1])

    y_true_eval_arr = np.concatenate(y_true_eval)
    prob_male_eval_arr = np.concatenate(prob_male_eval)

    y_pred_default = (prob_male_eval_arr >= 0.5).astype(np.int64)
    _evaluar_split("eval (umbral=0.50)", y_true_eval_arr, y_pred_default)

    best_threshold_info = _buscar_umbral_optimo(
        y_true=y_true_eval_arr,
        prob_male=prob_male_eval_arr,
        min_female_recall=MIN_FEMALE_RECALL_TARGET,
    )

    best_threshold = best_threshold_info["threshold"]
    print("\nBusqueda de umbral completada:")
    print(f"  - Umbral male seleccionado: {best_threshold:.2f}")
    print(f"  - Macro-F1: {best_threshold_info['f1_macro']:.4f}")
    print(f"  - Balanced Accuracy: {best_threshold_info['balanced_accuracy']:.4f}")
    print(f"  - Recall female: {best_threshold_info['recall_female']:.4f}")
    print(f"  - Recall male: {best_threshold_info['recall_male']:.4f}")
    print(f"  - Restriccion recall female usada: {best_threshold_info['constraint_used']}")

    y_pred_tuned = (prob_male_eval_arr >= best_threshold).astype(np.int64)
    _evaluar_split(
        f"eval (umbral ajustado={best_threshold:.2f})",
        y_true_eval_arr,
        y_pred_tuned,
    )

    # Se modifica el intercepto para que clf.predict() aplique este umbral.
    _aplicar_umbral_al_modelo(clf, best_threshold)

    print("\nTest set reservado (sin evaluacion final en esta corrida).")
    return clf, best_threshold_info
...
def main():
    print("=" * 60)
    print("CLASIFICADOR DE GENERO CON SAE (BALANCEADO)")
    print("=" * 60)
    ...
    for i, pooling_mode in enumerate(POOLING_MODES_TO_RUN, start=1):
        ...
        clf, threshold_info = entrenar_clasificador_streaming_balanceado(
            dataset=dataset,
            sae=sae,
            model=model,
            tokenizer=tokenizer,
        ...
        mode_output_path = f"modelos/clasificador_genero_balanceado_{pooling_mode}.pkl"
        joblib.dump(clf, mode_output_path)
        print(f"\nClasificador balanceado guardado en: {mode_output_path}")

        meta_output_path = f"modelos/clasificador_genero_balanceado_{pooling_mode}_meta.json"
        with open(meta_output_path, "w", encoding="utf-8") as f:
            json.dump(
                {
                    "pooling_mode": pooling_mode,
                    "female_weight_multiplier": FEMALE_WEIGHT_MULTIPLIER,
        ...
        print(f"Metadatos guardados en: {meta_output_path}")

        if i == 1:
            default_output_path = "modelos/clasificador_genero_balanceado.pkl"
            joblib.dump(clf, default_output_path)
            print(f"Clasificador balanceado por defecto actualizado en: {default_output_path}")
