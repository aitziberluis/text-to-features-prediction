...
# definir OUTPUT_DIR para evitar NameError
OUTPUT_DIR = "modelos/genero_qwen_sae_activaciones"

# crear el directorio si no existe antes de guardar el resumen
...
ACTIVATIONS_DIR = "/hdd/aitziber.l/activaciones_qwen_sae_genero"
...
def cargar_datos_genero() -> pd.DataFrame:
    ...
    df, _ = preparar_dataset_para_sae(path_comentarios=PATH_COMENTARIOS,path_autores=PATH_AUTORES,max_comments=MAX_COMMENTS,solo_genero_conocido=True,
    )
...
def _extraer_activaciones(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Optional[np.ndarray], int]:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        MODEL,
        device_map={"": DEVICE},
        torch_dtype=torch.float16 if torch.cuda.is_available() else torch.float32,  # Quadro RTX 8000 (Turing) sin BF16 nativo
    )
    model.eval()
    ...
    last_token_arr = np.memmap(
        ...
        dtype=np.float32, mode="w+", shape=(n, num_latents),
    )
    ...
    try:
        with torch.no_grad():
            for step, start in enumerate(range(0, n, EXTRACT_BATCH_SIZE)):
                ...
                tokens = tokenizer(
                    batch_texts,max_length=CONTEXT_LEN,truncation=True,padding=True,return_attention_mask=True,return_tensors="pt",
                )
                input_ids = tokens["input_ids"].to(model.device)
                attention_mask = tokens["attention_mask"].to(model.device)

                model._trunk(input_ids=input_ids, attention_mask=attention_mask)
                ...
                last_token_arr[start:end] = np.nan_to_num(last_pooled.float().cpu().numpy(), nan=0.0, posinf=0.0, neginf=0.0)
                mean_token_arr[start:end] = np.nan_to_num(mean_pooled.float().cpu().numpy(), nan=0.0, posinf=0.0, neginf=0.0)
...
def extraer_activaciones(
    ...
    if os.path.exists(meta_path):
        ...
        last_token = np.memmap(
            os.path.join(ACTIVATIONS_DIR, "last_token.mmap"),
            dtype=np.float32, mode="r", shape=(n, num_latents),
        )
...
def main():
    ...
    user_comment_pooling = None
    user_artifact = None
    if best_user_entry is not None:
        user_run_name, _ = best_user_entry
        user_artifact = trained_runs[user_run_name]
        user_comment_pooling = user_artifact.get("comment_pooling")

    # determinar comment_pooling necesario para el mejor comentario (si lo hay)
    best_comment_entry = best_per_level.get("comentario")
    comment_artifact = None
    comment_run_name = None
    if best_comment_entry is not None:
        comment_run_name, _ = best_comment_entry
        comment_artifact = trained_runs[comment_run_name]

    # una sola pasada de TEST recolectando lo necesario
    ...
    for start, end, last_np, mean_np in _stream_sae_features(
        ...
        if comment_artifact is not None:
            ...
            feats_scaled = scalers[pooling].transform(feats_by_pooling[pooling].astype(np.float32))
            comment_test_preds.append(comment_artifact["clf"].predict(feats_scaled))
    ...
    if comment_artifact is not None:
        ...
        m_c = evaluar(f"TEST {comment_run_name}", y_test_comments, y_test_pred_c)
        best_per_level_test["comentario"] = (comment_run_name, m_c)
        all_results[f"test_{comment_run_name}"] = m_c
    ...
    if user_artifact is not None:
        ...
        m_u = evaluar(f"TEST {user_run_name}", y_u_test, y_u_pred)
        best_per_level_test["usuario"] = (user_run_name, m_u)
        all_results[f"test_{user_run_name}"] = m_u
