...
def main() -> None:
    config = SaeInterpretabilityConfig(task_name="genero",dataset_kind="gender",cache_dir="/hdd/aitziber.l/activaciones_sae_gpt2_genero",splits_dir="data/splits_genero_70_15_15",output_dir="modelos/genero_sae_interpretabilidad",class_names=("female", "male"),top_k_latents=TOP_K,top_examples_per_latent=TOP_EXAMPLES,
    )
    ...
    _train_users, class_act_stats = _aggregate_user_features(df=df_train,tokenizer=tokenizer,model=model,sae=sae,hookpoint_module=hookpoint_module,num_latents=num_latents,pass_name="TRAIN",pooling="mean_of_mean",num_classes=len(config.class_names),
    )
