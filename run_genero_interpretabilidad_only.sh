#!/usr/bin/env bash
# Solo el paso de interpretabilidad SAE para la tarea de genero.
# Usa los modelos/splits ya existentes y la SAE; relanza unicamente
# clasificacion_genero/gpt_activaciones/interpretabilidad_genero_sae.py
# para regenerar los resultados con la version actualizada de
# interpretabilidad_sae_common.py (top latentes mas activos por clase).
#
# Uso directo:
...
MASTER_LOG="$LOG_DIR/run_genero_interp_$(date +%Y%m%d_%H%M%S).log"
LATEST_LINK="$LOG_DIR/run_genero_interp_latest.log"
SCRIPT_PATH="clasificacion_genero/gpt_activaciones/interpretabilidad_genero_sae.py"

mkdir -p "$LOG_DIR" "$HDD_DIR"
cd "$ROOT_DIR"

ln -sfn "$MASTER_LOG" "$LATEST_LINK"
printf '%s\n' "$$" > "$LOG_DIR/run_genero_interp.pid"
...
log "--- Iniciando $SCRIPT_PATH (solo interpretabilidad) ---"
START_SEC=$SECONDS
