...
def detectar_batch_size_optimo(
	...
	def _try_batch(bs: int):
		...
		util = peak / total_mem
		return util, peak
	# El forward-only subestima ~3x la VRAM real (backward necesita gradientes+ estados del optimizer). Escalamos el target para compensar.
...
def cargar_dataset_texto() -> Dataset:
	...
	La SAE es un modelo no supervisado, así que no necesita etiquetas; se entrena sobre todo el corpus de comentarios paraque luego pueda reutilizarse en tareas de género, edad, etc.
	"""
...
def preparar_modelo_y_datos(dataset: Dataset):
	...
	gpt = AutoModelForCausalLM.from_pretrained(MODEL,device_map={"": DEVICE},torch_dtype=torch.float16 if torch.cuda.is_available() else torch.float32,
	)
	gpt.eval()
	...
	cache_path = "/hdd/aitziber.l/tokenized_gpt2"
	if os.path.isdir(cache_path):
		...
		tokenized = dataset.map(
			_tokenize_fn,batched=True,batch_size=TOKENIZE_BATCH_SIZE,num_proc=TOKENIZE_NUM_PROC,load_from_cache_file=True,
		)
...
def entrenar_sae(dataset: Dataset):
	...
	sae_cfg = SaeConfig(d_in=hidden_size,num_latents=2**14,k=64,hookpoint=HOOKPOINT,
	)

	sae = Sae(sae_cfg, device=DEVICE)
	batch_size = detectar_batch_size_optimo(model=gpt,hookpoint=sae_cfg.hookpoint,context_len=CONTEXT_LEN,target_vram_util=TARGET_VRAM_UTIL,
	)
	...
	train_cfg = TrainConfig(wandb_project="tiny-sae-comments",wandb_name="sae-gpt2-comments",save_every_n_tokens=save_every,optimize_every_n_tokens=OPTIMIZE_EVERY_N_TOKENS,model_batch_size=batch_size,mask_first_n_tokens=1,save_repr_every_n_steps=SAVE_REPR_EVERY_N_STEPS,checkpoint_dir=CHECKPOINT_DIR,)
	...
	train_sae(sae=sae,model=gpt,token_iterator=tokenized,train_cfg=train_cfg,use_wandb=False,
	)
